...
<% if(!(request.getAttribute("user")==null)){
   ...
   wangyuandong.model.User u = (wangyuandong.model.User)request.getAttribute("user");
%>
...
<form method="post" action="updateUser">
   <label>Username : </label> <input type="text" name="username" placeholder="Username" value="<%=u.getUsername()%>"/>
    <label>Password : </label><input type="text" name="password" placeholder="password" value="<%=u.getPasswords()%>"/>
    <label>Email : </label><input type="text" name="email" placeholder="Email" value="<%=u.getEmails()%>"/><br/>
 	<label> Gender </label>
